...
public class FileClass {
	...
	public void fileWrite(String filename, String str[]){
		...
		File writeFile = new File("c:\\coffeeDB\\"+ filename + ".html");
	...
	public static void filePrint(){
		File tmpdir = new File("c:\\coffeeDB");
		File filelist[] = tmpdir.listFiles();
		
		for(int i=0; i<filelist.length; i++){
			// System.out.println(filelist[i]);
			if(filelist[i].isFile()){
				System.out.println("[팀명]" + filelist[i].getName());
			}
